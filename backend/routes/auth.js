  ...
  try {
    ...
    if (!process.env.TWILIO_SID || !process.env.TWILIO_AUTH) {
      ...
      try {
        ...
        console.error("⚠️ Twilio WhatsApp send failed (falling back to console log):", twilioErr.message);
        console.log("DEV MESSAGE (FALLBACK):");
        console.log(`Admission No: ${admissionNo}`);
        console.log(`Password: ${plainPassword}`);
      }
    }
  ...
  try {
    ...
    if (!process.env.TWILIO_SID || !process.env.TWILIO_AUTH) {
      ...
      try {
        ...
        console.error("⚠️ Twilio OTP send failed (falling back to console log):", twilioErr.message);
        console.log("DEV OTP (FALLBACK):", otp);
        return res.json({ message: "OTP sent (dev fallback mode)" });
      }
    }
